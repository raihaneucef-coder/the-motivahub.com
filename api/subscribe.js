...
export default async function handler(req, res) {
  ...
  // Fire-and-forget: respond 200 immediately, send email in background.
  // Vercel Hobby maxDuration = 10s. Resend usually <2s, but this guarantees
  // we never return 502 to the user if Vercel kills the function.
...
<html><body style="font-family:Georgia,serif;background:#faf7f2;color:#1a1a1a;padding:40px 20px;">
  ...
  // Background fetch — keep function alive until Resend finishes.
  // Vercel: res.waitUntil(promise) — function stays alive until promise resolves
  // This eliminates the 502 timeout error rate. We respond 200 immediately
  // and let Vercel keep the function warm for Resend to finish.
