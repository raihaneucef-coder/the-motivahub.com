...
- Name: verify-fixes.spec.ts >> Verify recent fixes >> contact page typography matches founder style
- Location: tests/verify-fixes.spec.ts:40:3
...
Locator: locator('.article-body .container > p').first()
Expected pattern: /Fraunces|serif/
Received string:  "\"JetBrains Mono\", \"IBM Plex Mono\", \"Courier New\", monospace"
Timeout: 5000ms

Call log:
  - Expect "toHaveCSS" locator('.article-body .container > p').first() with timeout 5000ms
  - waiting for locator('.article-body .container > p').first()
    14 × locator resolved to <p class="card-meta" data-i18n="contactPageKicker">CONTACT</p>
       - unexpected value ""JetBrains Mono", "IBM Plex Mono", "Courier New", monospace"
