...
(async () => {
  ...
  for (const p of pages) {
    ...
    // Get EN visible text (excluding scripts and styles)
    const enText = await page.$eval('body', el => {
    ...
    // Get FR visible text (excluding scripts and styles)
    const frText = await page.$eval('body', el => {
      const clone = el.cloneNode(true);
      clone.querySelectorAll('script, style, noscript').forEach(s => s.remove());
      return clone.textContent;
    });
    ...
    // Check for specific English phrases that should be translated (excluding script content)
  ...
  for (const [name, data] of Object.entries(results)) {
    const passed = data.visibleTags === 0 && !data.hasArabic && data.englishPhrases.length === 0 && data.englishLabels.length === 0;
    console.log(`${name}: ${passed ? 'PASS' : 'FAIL'} (tags: ${data.visibleTags}, arabic: ${data.hasArabic}, english: ${data.englishPhrases.length}, labels: ${data.englishLabels.length})`);
